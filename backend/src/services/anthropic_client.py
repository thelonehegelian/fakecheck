"""
Anthropic Claude API client for fact-checking operations using LangChain.
"""
...
class AnthropicClient:
    """Async client for Anthropic Claude API using LangChain."""
    ...
    async def analyze_news(
        ...
        """
        Analyze news content using Anthropic Claude via LangChain.
...
Citations: {citations}""",
        ...
        # Note: We use a subset of the FactCheckResponse plain dict structure for the return
        # but ask the LLM to match the schema
        ...
        try:
            logger.info(
                f"Sending analysis request to Anthropic Claude (LangChain) with model: {self.model}"
            )
            ...
            # Re-inject metadata that might be handled differently or needed specifically
            response_dict["model_used"] = self.model
            # Note: LangChain abstract usage metrics, but we can't easily get them from with_structured_output result directly
            # without callbacks. For now, we omit detailed usage stats or would need a callback handler.
            
            # Ensure citations are included if not present (though they are excluded above to avoid overwrite if needed)
            if citations:
    ...
    async def analyze_source_credibility(
        ...
        """
        Analyze source credibility using Anthropic Claude via LangChain.
        ...
        try:
            logger.info("Sending source analysis request to Anthropic Claude (LangChain)")
    ...
    async def extract_claims(
        ...
        """
        Extract claims from text using Anthropic Claude via LangChain.
        ...
        # Use ClaimExtractionResponse logic. 
        # Since logic expects a dict wrapping claims, we can define a simpler schema or use the nested one.
        # ClaimExtractionResponse has 'extracted_claims' list. Let's use that.
        # However, the previous implementation expected a tool output that might differ slightly.
        # Let's ensure we match the keys: 'claims' (list) and 'summary' (string).
        # We'll define a quick customized schema for this specific tool interaction if simpler.
        # Or better, use a composed Pydantic model.
        ...
        try:
            logger.info("Sending claim extraction request to Anthropic Claude (LangChain)")
            result = await chain.ainvoke({"prompt": prompt})
            
            # The existing code expects a dict with "claims" (list of dicts) and "summary".
            # result.claims is a list of ExtractedClaim objects.
            response_dict = {
    ...
    async def analyze_claim_verifiability(
        ...
        """
        Analyze how a claim can be verified using Anthropic Claude via LangChain.
        ...
        try:
            logger.info("Sending verifiability analysis request to Anthropic Claude (LangChain)")
            result = await chain.ainvoke({"prompt": prompt})
    ...
    def _handle_langchain_exception(self, e: Exception) -> None:
        ...
        """
        error_msg = str(e)
        logger.error(f"LangChain/Anthropic error: {error_msg}")
        
        # Check for rate limit
        if "rate_limit" in error_msg.lower() or "429" in error_msg:
             # Try to parse retry after header if available in error message, usually hard
        ...
        # Generic API errors
        if "api_error" in error_msg.lower() or "400" in error_msg: # simplistic check
             raise AnthropicAPIError(f"Anthropic API request failed: {error_msg}")
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "structured_output_available": True, # Assumed with LangChain
            }
